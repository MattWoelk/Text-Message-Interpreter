#this program should find every word that could be spelled with a given telephone number
#(or within a telephone number)
...
def possibilities(wordNums)
  ...
  while (line = file.gets)
    if line.nil? == false && line.chomp.length == wordNums.length && (findWords(line) <=> wordNums) == 0
...
if __FILE__ == $0
  ...
  while jumble.length != 7
    puts "enter seven numbers"
    jumble = gets.chomp
  end
